// Lumixo web — heavy call engine (WebRTC + overlay UI).
// Loaded lazily after first paint so motion/WebRTC stay off the critical path.
// Registers startCall/busy with the light CallProvider via onApiReady.
//
...
// see 'accepted' before creating and sending its SDP offer.
...
export function CallEngine({ onApiReady }: { onApiReady: (api: CallApi) => void }) {
  ...
  // Publish API to light CallProvider (stable children tree — no remount).
  useEffect(() => {
    onApiReady({ startCall, busy: phase !== 'idle' });
  }, [startCall, phase, onApiReady]);
