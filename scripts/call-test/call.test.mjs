...
//
// Timeouts MUST match mobile/src/calls/webrtc.ts:
//   CONNECT_TIMEOUT_MS = 50_000
//   DISCONNECT_TEARDOWN_MS = 20_000
//   ICE_RESTART_GRACE_MS = 3_500
//   MAX_ICE_RESTARTS = 5
...
test('SIGNALING: ready→offer→answer→candidate exchange completes; both reach stable', async () => {
  ...
  console.log(
    '  ICE candidates added — caller:',
    pcA.addedCandidates.length,
    'callee:',
    pcB.addedCandidates.length,
  );
  ...
  assert.ok(
    pcA.addedCandidates.length >= 1 && pcB.addedCandidates.length >= 1,
    'ICE candidates crossed both ways',
  );
  assert.ok(
    caller.events.includes('localStream') && callee.events.includes('localStream'),
...
test('CONNECT: firing iceConnectionState=connected clears "Connecting…" on both peers', async () => {
  const { caller, callee, pcA, pcB } = await handshake('audio');
  pcA.__fireIce('connected');
  pcB.__fireIce('connected');
...
test('STUCK-BUG FIX: offer permanently dropped → was stuck forever → watchdog now ENDS at 50s', async () => {
  ...
  mock.timers.tick(8400);
  await flush(); // heartbeat window elapses, still wedged
  console.log(
    '  at 8.4s — connected:',
    caller.events.includes('connected'),
    'ended:',
    caller.events.includes('ended'),
    '(stuck)',
  );
  assert.ok(!caller.events.includes('ended'), 'still stuck after heartbeat gives up');

  mock.timers.tick(CONNECT_TIMEOUT_MS - 1 - 8400);
  await flush();
  assert.ok(
    !caller.events.includes('ended'),
    `still stuck at t=${CONNECT_TIMEOUT_MS - 1}ms (proves it would hang without watchdog)`,
  );

  mock.timers.tick(2);
  await flush(); // cross the 50s watchdog
  console.log(
    '  at 50s — ended:',
    caller.events.includes('ended'),
    '| ever connected:',
...
test('WATCHDOG cleared on success: a call that connects does NOT get killed at 50s', async () => {
  const { caller, pcA } = await handshake('audio');
  pcA.__fireIce('connected');
  await flush();
  mock.timers.tick(60000);
  await flush(); // long past the watchdog
...
test('ICE FAILED: attempts ICE restart then ends after MAX_ICE_RESTARTS exhausted', async () => {
  const { caller, pcA } = await handshake('audio');
  // Production does NOT immediately end on first ice=failed — it restarts up to 5 times.
  for (let i = 0; i < MAX_ICE_RESTARTS; i++) {
    pcA.__fireIce('failed');
    await flush();
    assert.ok(
      !caller.events.includes('ended'),
      `ICE restart #${i + 1} should not end yet (MAX=${MAX_ICE_RESTARTS})`,
    );
  }
  // One more failed edge after restarts exhausted → end
  pcA.__fireIce('failed');
  await flush();
  console.log('  events after ICE restarts exhausted:', caller.events.join(','));
  assert.ok(caller.events.includes('ended'), 'ICE failure ends the call after restart budget');
});

test('RECONNECT grace: transient disconnect recovers; sustained disconnect ends after 20s', async () => {
  const { caller, pcA } = await handshake('audio');
  pcA.__fireIce('connected');
  await flush();
  pcA.__fireIce('disconnected');
  await flush(); // blip
  mock.timers.tick(5000); // past ICE_RESTART_GRACE (3.5s) but within 20s teardown
  pcA.__fireIce('connected');
  await flush(); // recovered within grace
  mock.timers.tick(DISCONNECT_TEARDOWN_MS);
  await flush();
  ...
  pcA.__fireIce('disconnected');
  await flush();
  mock.timers.tick(DISCONNECT_TEARDOWN_MS + 1);
  await flush();
  console.log('  after sustained disconnect: ended?', caller.events.includes('ended'));
  assert.ok(
    caller.events.includes('ended'),
    `sustained disconnect ended after ${DISCONNECT_TEARDOWN_MS}ms grace`,
  );
});
...
test('CONTROLS: mute / speaker / video toggle / switchCamera act on tracks + audio routing', async () => {
  ...
  console.log(
    '  speaker now:',
    spk,
    '| InCallManager speaker calls:',
    ICM.__log.filter((l) => l[0] === 'speaker').length,
  );

  const videoOn = s.toggleVideo();
  const videoTrack = pcA.tracks.map((t) => t.track).find((t) => t.kind === 'video');
  assert.equal(videoOn, false);
  assert.equal(videoTrack.enabled, false);

  // Production prefers getUserMedia + replaceTrack (facing flip); _switchCamera is fallback.
  assert.equal(s.getFacing(), 'user');
  await s.switchCamera();
  await flush();
  const facingAfter = s.getFacing();
  const videoSender = pcA.getSenders().find((x) => x.track && x.track.kind === 'video');
  const replaced = videoSender?.__replaced ?? 0;
  const switchedFallback = (videoTrack.__switched || 0) > 0;
  console.log(
    '  facing:',
    facingAfter,
    '| replaceTrack count:',
    replaced,
    '| _switchCamera fallback:',
    switchedFallback,
  );
  assert.equal(facingAfter, 'environment', 'camera facing flipped user→environment');
  assert.ok(
    replaced >= 1 || switchedFallback,
    'switchCamera used replaceTrack or _switchCamera fallback',
  );

  await s.switchCamera();
  await flush();
  assert.equal(s.getFacing(), 'user', 'camera facing flipped back to user');
});
